    ...
    public class EngineContext
    {
        ...
        public static IEngine CreateEngineInstance(Config config, IDependencyManager dependencyManager)
        {
            if (config != null && !string.IsNullOrEmpty(config.EngineType))
            {
                ...
                {
                    throw new ConfigurationErrorsException("The type '" + engineType + "' doesn't implement 'Aprimo.Utility.Framework.IEngine' and cannot be configured in /configuration/ns/engine[@engineType] for that purpose.");
                }
